...
public class FIXTrackerApplication extends MessageCracker 
    ...
    public void onMessage( quickfix.fix42.NewOrderSingle message, 
        ...
        if ( executorStarted ) {
            ...
            try {
                autoAck = settings.getBool("FIXTrackerAutoAcknowledge");
            } catch ( Exception e ) {}
            if ( autoAck ) {
                acknowledge( order );
            }
    ...
    public void onMessage( quickfix.fix42.OrderCancelRequest message, 
        ...
        try {
            autoPending = settings.getBool("FIXTrackerAutoPendingCancel");
            autoCancel = settings.getBool("FIXTrackerAutoCancel");
        } catch ( Exception e ) {}
        if ( autoPending ) {
            pendingCancel(order);
        }
    ...
    public void onMessage( quickfix.fix42.OrderCancelReplaceRequest message, 
        ...
        boolean autoCancel = false;
        try {
            autoPending = settings.getBool("FIXTrackerAutoPendingReplace");
            autoCancel = settings.getBool("FIXTrackerAutoReplace");
        } catch ( Exception e ) {}
        if ( autoPending ) {
    ...
    public void sendMessage( Message message ) {
        ...
        try {
            ...
            sendoboCompID = settings.getBool("FIXTrackerSendOnBehalfOfCompID");
            sendoboSubID = settings.getBool("FIXTrackerSendOnBehalfOfSubID");
        } catch ( Exception e ) {}
    ...
    public void sendIOI( IOI ioi ) {
        ...
        quickfix.fix42.IndicationofInterest fixIOI = 
         ...
         //    new quickfix.fix42.IndicationofInterest(ioiID, ioiType, symbol, side, shares);
        ...
        Instrument instrument = 
                FIXTracker.getInstruments().getInstrument(ioi.getSymbol());
        ...
        ValidUntilTime validTime = new ValidUntilTime( );
    ...
    public class IOIsender implements Runnable {
    	...
    	public IOIsender ( Integer delay, String symbol, String securityID ) {
            instruments = FIXTracker.getInstruments();
