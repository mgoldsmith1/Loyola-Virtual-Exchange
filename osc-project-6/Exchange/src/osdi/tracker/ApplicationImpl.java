...
public class ApplicationImpl extends MessageCracker 
    ...
    public void onMessage( quickfix.fix42.NewOrderSingle message, 
        ...
        if ( executorStarted ) {
            ...
            try {
                autoAck = settings.getBool("MarketExchangeSimAutoAcknowledge");
            } catch ( Exception e ) {}
            if ( autoAck ) {
                acknowledge( order );
            }
    ...
    public void onMessage( quickfix.fix42.OrderCancelRequest message, 
        ...
        try {
            autoPending = settings.getBool("MarketExchangeSimAutoPendingCancel");
            autoCancel = settings.getBool("MarketExchangeSimAutoCancel");
        } catch ( Exception e ) {}
        if ( autoPending ) {
            pendingCancel(order);
        }
    ...
    public void onMessage( quickfix.fix42.OrderCancelReplaceRequest message, 
        ...
        boolean autoCancel = false;
        try {
            autoPending = settings.getBool("MarketExchangeSimAutoPendingReplace");
            autoCancel = settings.getBool("MarketExchangeSimAutoReplace");
        } catch ( Exception e ) {}
        if ( autoPending ) {
    ...
    public void sendMessage( Message message ) {
        ...
        try {
            ...
            sendoboCompID = settings.getBool("MarketExchangeSimSendOnBehalfOfCompID");
            sendoboSubID = settings.getBool("MarketExchangeSimSendOnBehalfOfSubID");
        } catch ( Exception e ) {}
    ...
    public void sendIOI( IOI ioi ) {
        ...
        Instrument instrument = 
                MarketExchangeSim.getInstruments().getInstrument(ioi.getSymbol());
    ...
    public class IOIsender implements Runnable {
    	...
    	public IOIsender ( Integer delay, String symbol, String securityID ) {
            instruments = MarketExchangeSim.getInstruments();
    	...
    	public void sendRandomIOI() {
            ...
            try {
                pricePrecision = 
                        (int)settings.getLong("MarketExchangeSimPricePrecision");
            } catch ( Exception e ) {}
